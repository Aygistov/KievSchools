...
Examples:
```
  // for requests to an `api` base URL that gets schools from a 'schools' collection 
  GET api/schools          // all schools
  GET api/schools/42       // the school with id=42
  GET api/schools?name=^j  // 'j' is a regex; returns schools whose name starting with 'j' or 'J'
  GET api/schools.json/42  // ignores the ".json"
```
...
export class InMemSchoolService implements InMemoryDbService {
  createDb() {
    let schools = [
      { id: 1, name: 'Windstorm' },
      { id: 2, name: 'Bombasto' },
...
import { InMemSchoolService } from '../app/school.service';

@NgModule({
 imports: [
   HttpClientModule,
   HttpClientInMemoryWebApiModule.forRoot(InMemSchoolService),
   ...
 ],
 ...
})
...
imports: [
  HttpModule,
  HttpInMemoryWebApiModule.forRoot(InMemSchoolService),
  ...
],
...
})
...
imports: [
  HttpModule,
  HttpClientModule,
  InMemoryWebApiModule.forRoot(InMemSchoolService),
  ...
],
...
})
...
[Tour of Schools](https://angular.io/tutorial/toh-pt6). 
...
```ts
  InMemoryWebApiModule.forRoot(InMemSchoolService, { delay: 500 }),
```
...
```ts
  InMemoryWebApiModule.forRoot(InMemSchoolService, { delay: 0 }),    // no delay
  InMemoryWebApiModule.forRoot(InMemSchoolService, { delay: 1500 }), // 1.5 second delay
```
...
Format: `/app/schools/?propertyName=regexPattern`

The following example matches all names start with the letter 'j'  or 'J' in the schools collection.
...
http
  ...
  // when command finishes, get schools
  .concatMap(
    ()=> http.get<Data>('api/schools')
        .map(data => data.data as School[])
  )

  // execute the request sequence and 
  // do something with the schools
  .subscribe(...)
```
...
The `SchoolInMemDataService` class (in `src/app/school-in-mem-data.service.ts`) is a School-oriented `InMemoryDbService`
such as you might see in an HTTP sample in the Angular documentation.

The `SchoolInMemDataOverrideService` class (in `src/app/school-in-mem-data-override.service.ts`)
demonstrates a few ways to override methods of the base `SchoolInMemDataService`.
...
It's an Angular data service (`SchoolService`) and a bunch of tests.
