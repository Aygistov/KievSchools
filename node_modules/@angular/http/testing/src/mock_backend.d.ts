 ...
 *     return this.http.get('myservices.de/api/schools')
 *         .toPromise()
 ...
 * }
 *
 * describe('MockBackend SchoolService Example', () => {
 *   beforeEach(() => {
 ...
 *   });
 *
 *   it('getSchools() should query current service url', () => {
 *     this.schoolService.getSchools();
 *     expect(this.lastConnection).toBeDefined('no http service connection at all?');
 *     expect(this.lastConnection.request.url).toMatch(/api\/schools$/, 'url invalid');
 *   });
 *
 *   it('getSchools() should return some schools', fakeAsync(() => {
 *        let result: String[];
 *        this.schoolService.getSchools().then((schools: String[]) => result = schools);
 *        this.lastConnection.mockRespond(new Response(new ResponseOptions({
 *          body: JSON.stringify({data: [HERO_ONE, HERO_TWO]}),
 *        })));
 *        tick();
 *        expect(result.length).toEqual(2, 'should contain given amount of schools');
 *        expect(result[0]).toEqual(HERO_ONE, ' HERO_ONE should be the first school');
 *        expect(result[1]).toEqual(HERO_TWO, ' HERO_TWO should be the second school');
 *      }));
 *
 *   it('getSchools() while server is down', fakeAsync(() => {
 *        let result: String[];
 *        let catchedError: any;
 *        this.schoolService.getSchools()
 *            .then((schools: String[]) => result = schools)
